<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-button type="text" @click="copy(tableData.data,scope.row)">复制</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column v-if="loginType=='admin'" align="center" label="提成比例">
        <template slot-scope="scope">
          ...
          >
            ...
            <i slot="reference" class="el-icon-setting" style="cursor: pointer;" />
          </el-popover>
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          <el-row :gutter="5">
            ...
            <el-col :span="14">
              <el-button v-if="loginType=='union'" size="mini" @click="AddSecond(scope.row)">绑定公会管理员</el-button>
            </el-col>
            <el-col :span="14">
            ...
            <el-col :span="14">
              <el-button v-if="loginType=='admin'" :loading="tableData.loading" style="margin-top:5px" size="mini" type="primary" @click="hotRatioClick(scope.row)">操作热度</el-button>
            </el-col>
        ...
        <el-row :gutter="10">
          <el-col :span="22">
            ...
            <el-form-item label="信息描述">
              <el-input v-model="form.tradeInfo" type="textarea" :autosize="{ minRows: 2, maxRows: 4}" placeholder="请输入信息描述" />
            </el-form-item>
        ...
        <el-row :gutter="10">
          <el-col :span="22">
            ...
            <el-form-item label="开户行所在地">
              <el-input v-model="forms.bankAdr" placeholder="请输入内容" type="textarea" :autosize="{ minRows: 2, maxRows: 4}" />
            </el-form-item>
            <el-form-item label="开户行详细地址">
              <el-input v-model="forms.bankAdrName" placeholder="请输入内容" type="textarea" :autosize="{ minRows: 2, maxRows: 4}" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    ...
    <!-- 操作系数 -->
    <el-dialog :visible.sync="dialogVisible.hotRatio" width="500px" title="编辑热门扶持系数" center style="z-index: 9999">
      <el-form label-position="right" label-width="120px" :model="formHotRatio" size="mini">
        <el-row :gutter="10">
          <el-col :span="22">
            <el-form-item label="热门操作系数">
              <el-input v-model="formHotRatio.hotRatio" type="number" :step="0.1" min="0" />
            </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button
          size="small"
          @click="dialogVisible.hotRatio=false"
        >取 消</el-button>
        <el-button type="primary" :loading="tableData.loading" size="small" @click="hotRatioSubmit">提交</el-button>
      </div>
    </el-dialog>
...
<script>
import { tradeList, updateTrade, deleteBatch, tradeCash, updateAdmin, editPwd, hotRatio } from '@/api/union'
...
export default {
  ...
  methods: {
    ...
    // 操作热度
    hotRatioClick(row) {
      this.dialogVisible.hotRatio = true
      this.hotRatioId = row.id
      this.formHotRatio.hotRatio = row.hotRatio
    },
    hotRatioSubmit() {
      this.tableData.loading = true
      const _form = Object.assign({
        id: this.hotRatioId,
        hotRatio: this.formHotRatio.hotRatio
      })
      hotRatio(_form).then(res => {
        this.$message.success(res.message)
        this.fetchData()
        this.dialogVisible.hotRatio = false
      }).finally(_ => {
        this.tableData.loading = false
      })
    }
