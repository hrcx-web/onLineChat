<template>
  <div class="app-container">
    ...
    <div class="top">
      ...
      <div class="top-right">
        <input id="up" ref="file" style="display:none;" type="file" class="form-control" @change="importFile">
        <!-- <label for="up">
          <span class="importButton">导入</span>
        </label> -->
        <el-button type="success" size="small" @click="importIphone">解析并导入</el-button>
...
<script>
import { phoneInfoList, updatePhoneInfo, readFile, deleteBatch, exportXls, importExcel } from '@/api/messages'
...
export default {
  ...
  methods: {
    ...
    // 导出
    excelDow() {
      const _forms = Object.assign({
        carrier: this.filter.carrier,
        province: this.filter.province
      }, this.filter)
      exportXls(_forms).then(res => {
        const content = res
