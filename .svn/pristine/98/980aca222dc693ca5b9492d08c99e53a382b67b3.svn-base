<template>
  <div class="app-container">
    ...
    <div class="top">
      <div class="top-left">惩戒期主播列表</div>
      <div class="top-right">
      ...
      <el-form label-position="right" label-width="100px" :model="communityForm" size="mini">
        ...
        <el-form-item v-if="communityForm.type == 2" label="视频">
          <div class="icon-container" @click="fakeClick('upload')">
            ...
            <video v-if="communityForm.infoUrl && communityForm.type == 2" :src="form.infoUrl" style="width: 100%; height: 100%" controls />
...
<script>
import { query, updateUser, uploadQNImg, queryLabelInUse, getUserAuthInfo, addOrEdit, authUserInfo, userQueryById, webUserRatio, userDelete } from '@/api/user'
...
export default {
  ...
  methods: {
    ...
    submitCommunity() {
      ...
      if (_form.type === '2') {
        _form.coverUrl = this.communityForm.infoUrl + '?vframe/jpg/offset/0'
      }
    ...
    deleteFileOrDirectory() {
      ...
      this.$confirm('确定要删除选中的惩戒期主播列表信息吗?', '提示')
        .then(() => {
          userDelete({
            ids: ids
          }).then((data) => {
